"""
# Step 1a: Heston Model Calibration using Lewis (2001) Approach 
"""
...
"""Secondly, we define our option pricing functions. This function will be used in pricing European call option in B96 
Model via Lewis (2001)."""
...
We need to create a function that yields the model values under the different parameters obtained in the calibration process 
via Lewis (2001).
"""
...
"""# 2b. Bates Model Calibration - Carr-Madan """
...
**Objective:**
The primary goal of this task was to calibrate the Bates (1996) stochastic volatility model with jumps to the observed market prices 
for SM Energy options with a 60-day maturity.

**Methodology:**
1.  **Model Selection:** The Bates model was chosen as it extends the Heston model to account for sudden, large price movements (jumps),
which are a known feature of equity markets. This adds three parameters to the standard Heston model: jump intensity (lambda), mean jump
size (mu), and jump volatility (delta).

2.  **Pricing Engine:** 
We employed the Carr-Madan (1999) pricing method, which utilizes the Fast Fourier Transform (FFT).This technique is highly efficient 
for calibration as it can price a wide range of options simultaneously, significantly speeding up the optimization process compared to
methods like Monte Carlo simulation.

3.  **Error Minimization:** 
The calibration was performed by minimizing the Mean Squared Error (MSE) between the prices generated by our model and the actual 
prices observed in the market. The optimization was applied to both call and put options simultaneously to
ensure a robust fit across the entire volatility smile. Put prices were derived from call prices using the put-call parity
relationship.

**Results:**
The optimization algorithm successfully converged to a set of stable parameters with a final MSE of {final_error:.4f}.The calibrated
parameters are listed above. The resulting values are financially sensible; for instance, the negative correlation (rho) of 
{calibrated_params[3]:.2f} reflects the well-known leverage effect where volatility tends to increase as stock prices fall.

**Fit Quality:**
As shown in the graphs above, the calibrated Bates model provides an excellent fit to the market data for both call and put options. 
The model successfully captures the curvature of the volatility smile implied by the market prices, demonstrating its effectiveness for 
this dataset.
"""
...
**Objective:**
The goal of this task was to calibrate the Cox-Ingersoll-Ross (CIR) interest rate model to the current Euribor term structure to 
provide insights on future interest rate movements.

**Methodology:**
1.  **Term Structure Construction:** We began with the provided market Euribor rates for maturities ranging from 1 week to 12 months. 
These discrete points represent the current yield curve.

2.  **Interpolation:** To create a continuous and smooth yield curve, which is necessary for a robust calibration, we used 
the **cubic spline interpolation method**. This technique fits a series of piecewise cubic polynomials to the market data, 
ensuring a smooth transition between points. We interpolated to generate a high-frequency term structure with weekly data points over
a one-year horizon.

3.  **Model and Error Function:** 
The CIR model is a mean-reverting model for short-term interest rates. We used its closed-form solution for zero-coupon bond prices 
to derive a theoretical yield curve. The calibration was then set up as an optimization problem to find the CIR parameters—**kappa 
(κ)**, the speed of mean reversion; **theta (θ)**, the long-run mean rate; and **sigma (σ)**, the volatility—that minimized the Mean 
Squared Error (MSE) between the model-generated yields and the interpolated market yields.

4.  **Feller Condition:** A crucial constraint, the Feller condition (2κθ ≥ σ²), was enforced during optimization. This ensures that
the interest rate process in the model will not become negative, which is a key theoretical advantage of the CIR model.
...
**Fit Quality:**
The accompanying graph visually demonstrates the quality of the calibration. The calibrated CIR model's yield curve provides a 
very close fit to the interpolated market curve. This indicates that the CIR model, with the parameters we found, is effective at
describing the current term structure of Euribor rates. These parameters can now be confidently used for the Monte Carlo simulation
in the next step.
"""
